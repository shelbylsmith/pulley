...
- **Slack redelivery dedupe.** Slack redelivers events not acked within 3 seconds (a cold start is enough to trigger it). Each delivery races to claim its `event_id` in `slack_event_claims`; only the winner processes, so retries — even ones racing an in-flight original — can't double-post. A failed delivery releases its claim so the next Slack retry can rescue the event.
